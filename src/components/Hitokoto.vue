...
import { getHitokoto, getNsmaoQuotes } from "@/api";
...
const getHitokotoData = async () => {
  try {
    // 优先使用用户提供的一言 API
    console.log("使用用户提供的一言 API");
    const result = await getNsmaoQuotes();
    console.log(result);
    if (result.code === 200) {
      hitokotoData.text = result.data;
      hitokotoData.from = "一言API";
    } else {
      console.log("用户提供的一言 API 调用失败，尝试使用备用接口");
      // 如果用户提供的 API 失败，使用原有的一言 API
      const hitokotoResult = await getHitokoto();
      hitokotoData.text = hitokotoResult.hitokoto;
      hitokotoData.from = hitokotoResult.from;
    }
  } catch (error) {
    ElMessage({
      message: "一言获取失败",
